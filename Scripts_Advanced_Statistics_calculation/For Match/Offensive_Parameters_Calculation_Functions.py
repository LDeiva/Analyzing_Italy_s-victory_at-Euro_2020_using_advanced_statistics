...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 24 16:09:24 2022
...
"""Qui metto le funzioni che non uso per varie ragioni"""

#Calcolo differenza XG e gol   
def XG_GOAL_difference(df,teamname):
    #calcolo differenza xg e gol su azione
    dfnp=df[(df['team_name']==teamname)  & (df['shot_type_name']=='Open Play')  & (df['period']<=4)]
    npXG=0
    np_GOL=0
    for i in range(len(dfnp['type_name'])):
        if dfnp['type_name'].iloc[i]=='Shot':
            xg=dfnp['shot_statsbomb_xg'].iloc[i]
            npXG+=xg
            if dfnp['shot_outcome_name'].iloc[i]=='Goal':
                np_GOL+=1
    np_difference=np_GOL-npXG

    #calcolo differenza xg e gol in set pieces
    dfsp=df[(df['team_name']==teamname)  & (df['shot_type_name']!='Open Play') & (df['shot_type_name']!='Penalty')]
    spXG=0
    sp_GOL=0
    for i in range(len(dfsp['type_name'])):
        if dfsp['type_name'].iloc[i]=='Shot':
            xg=dfsp['shot_statsbomb_xg'].iloc[i]
            spXG+=xg
            if dfsp['shot_outcome_name'].iloc[i]=='Goal':
                sp_GOL+=1
    p_difference=sp_GOL-spXG

    #calcolo differenza xg totali e gol totali esclusi i rigori

    tot_GOL=np_GOL+sp_GOL
    tot_XG=npXG+spXG


    tot_difference=tot_GOL-tot_XG
    
    return np_difference,p_difference,tot_difference
 

"""Calcolo la percentuale di conversione dei tiri totale e solo quelli in porta sul numero di gol"""
def shots_conversion_percentage(df,teamname):
    #calcolo percentuale conversione per i tiri su azione
    dfnp=df[(df['team_name']==teamname)  & (df['shot_type_name']=='Open Play')  & (df['period']<=4)]
    np_GOL_n=0
    np_shots=0
    for i in range(len(dfnp)):
        ...
        if np_shots!=0:
            np_shots_conversion=(np_GOL_n/np_shots)*100
        else:
            np_shots_conversion=0
              
    #calcolo percentuale conversione per i tiri non su azione
    dfp=df[(df['team_name']==teamname)  & (df['shot_type_name']!='Open Play') & (df['shot_type_name']!='Penalty')  & (df['period']<=4)]
    sp_GOL_n=0
    sp_shots=0
    for i in range(len(dfnp)):
        if dfp['type_name'].iloc[i]=='Shot':
            sp_shots+=1
            if dfp['shot_outcome_name'].iloc[i]=='Goal':
                sp_GOL_n+=1
                
        if sp_shots!=0:
            sp_shots_conversion=(sp_GOL_n/sp_shots)*100
        else:
            sp_shots_conversion=0
    
    #calcolo percentuale conversione per i tiri totali
    tot_GOL_n=np_GOL_n+sp_GOL_n
    tot_shots=np_shots+sp_shots
    
    tot_shots_conversion=(tot_GOL_n/tot_shots)*100
    return np_shots_conversion,sp_shots_conversion,tot_shots_conversion



"""Calcolo i tocchi in area.
non calcolabile"""
def Touches_in_Box(df,team):
    team_touch=df[df['team_name']==team]
    Touch=0
    for i in range(len(team_touch)):
        if team_touch['type_name'].iloc[i]=='Pass' or team_touch['type_name'].iloc[i]=='Ball Receipt*' or team_touch['type_name'].iloc[i]=='Shot' or team_touch['type_name'].iloc[i]=='Carry' or team_touch['type_name'].iloc[i]=='Dribble':
            if team_touch['location'].iloc[i][0]>=102 and 18<=team_touch['location'].iloc[i][1]<=62:
                Touch+=1
    return Touch
...
"""C) Statistiche XG, Tiri, Gol e Assist"""
...
def Passes_and_cross_into_the_Box_for_player(df,lista):
    ...
    for p in lista:
        ...
        b_c_percentage_list.append(percentage(c_b, n_p_b))
...
"""Calcolo i passaggi, che non sono cross, effettuati entro 20 metri dalla porta avversaria."""
def Deep_Pass_Completions_for_players(df,lista):
...
"""Calcolo i cross, effettuati entro 20 metri dalla porta avversaria."""
def Deep_Cross_Completions_for_players(df,lista):
